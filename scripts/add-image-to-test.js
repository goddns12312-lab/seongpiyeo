...
const TEST_LISTING_ID = 'a9996682-69af-4b56-ae41-f86226171312';
...
async function main() {
  ...
  try {
    console.log('🚀 원주관설동 매물에서 이미지 수집 중\n');
    ...
    for (let imgIdx = 0; imgIdx < Math.min(imageUrls.length, 9); imgIdx++) {
      try {
        ...
        const filename = `listing-test-${imgIdx + 1}-${Date.now()}.jpg`;
    ...
    if (uploadedUrls.length > 0) {
      console.log('🔄 테스트 매물에 이미지 추가 중...\n');

      // 기존 이미지 삭제
      await supabase
        .from('listing_images')
        .delete()
        .eq('listing_id', TEST_LISTING_ID);
    ...
    console.log(`   http://localhost:3001/listings/${TEST_LISTING_ID}`);
