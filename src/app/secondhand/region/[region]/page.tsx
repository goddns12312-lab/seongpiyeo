...
export default async function SecondhandRegionPage({ params }: Props) {
  const { region } = await params;
  const decodedRegion = decodeURIComponent(region);

  const items = await fetchSecondhandItems({ region: decodedRegion });
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="bg-bg-secondary border border-border-light rounded-lg p-4">
            ...
            <p className="text-2xl font-bold text-gold">{items.length}개</p>
          </div>
          <div className="bg-bg-secondary border border-border-light rounded-lg p-4">
